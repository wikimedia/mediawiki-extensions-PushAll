...
final class PushFunctions {
	...
	/**
	 * Function to change the keys of $egPushAllLoginUsers and $egPushAllLoginPasswords
	 * from target url to target name using the $egPushAllTargets array.
	 *
	 * @since 0.5
	 *
	 * @param array &$arr
	...
	public static function flipKeys( array &$arr, $id ) {
		...
		if ( !in_array( $id, $handledArrays ) ) {
			...
			if ( !$config->has( "Targets" ) ) {
				// throw new MWException( "$egPushAllPushTargets is not precised in the bundle/Push/extension.json
			...
			foreach ( $arr as $key => $value ) {
				if ( array_key_exists( $key, $egPushAllTargets ) ) {
					$flipped[$egPushAllTargets[$key]] = $value;
				}
			}
